...
BASE_URL = "https://catxgnn.yao.care"
...
def generate_capability_statement() -> dict:
    ...
    return {
        ...
        "id": "catxgnn-fhir-server",
        "url": f"{BASE_URL}/fhir/metadata",
        "version": "1.0.0",
        "name": "CaTxGNNFHIRServer",
        "title": "Canada TxGNN FHIR Server",
        "status": "active",
        "experimental": True,
        "date": datetime.now().strftime("%Y-%m-%d"),
        "publisher": "Yao.Care - CaTxGNN Project",
        "description": "FHIR R4 API for Canada drug repurposing predictions based on TxGNN knowledge graph",
        "kind": "instance",
        "fhirVersion": "4.0.1",
        "format": ["json"],
        "rest": [{
...
def generate_medication_knowledge(
    ...
    return {
        ...
        "regulatory": [{
            "regulatoryAuthority": {
                "display": "Health Canada"
            },
            "identifier": [{
                "system": f"{BASE_URL}/ca-din",
                "value": license_id
            }]
        }],
        "extension": [{
...
def main():
    print("=" * 60)
    print("Generating FHIR R4 Resources - CaTxGNN")
    print("=" * 60)
    print()
    ...
    bundle = {
        ...
        "link": [
            {"relation": "self", "url": f"{BASE_URL}/fhir/Bundle/catxgnn-summary"}
        ],
        "entry": [
