---
layout: default
nav_exclude: true
title: Carboxymethylcellulose Sodium
drugbank_id: DB11059
evidence_level: L5
permalink: /drugs/db11059/
---

# Carboxymethylcellulose Sodium

## Basic Information

| Item | Value |
|------|-------|
| DrugBank ID | [DB11059](https://go.drugbank.com/drugs/DB11059) |
| Evidence Level | L5 (Computational Prediction) |
| Number of Predicted Indications | 17 |

## Predicted Indications (TxGNN)

The following are potential new indications predicted by the TxGNN model. Higher scores indicate higher predicted relevance.

| # | Indication | Source |
|---|------------|--------|
| 1 | irritable bowel syndrome | KG + DL |
| 2 | open-angle glaucoma | KG + DL |
| 3 | primary hereditary glaucoma | KG + DL |
| 4 | congenital hypotrichosis milia | KG + DL |
| 5 | alopecia | KG + DL |
| 6 | hypotrichosis simplex of the scalp | KG + DL |
| 7 | diffuse alopecia areata | KG + DL |
| 8 | cauda equina syndrome | KG + DL |
| 9 | gastroenteritis | KG + DL |
| 10 | enterocolitis (disease) | KG + DL |
| 11 | bile duct neoplasm | KG + DL |
| 12 | headache disorder | KG + DL |
| 13 | small intestine cancer | KG + DL |
| 14 | large intestine disease | KG + DL |
| 15 | biliary tract disease | KG + DL |
| 16 | small intestine disease | KG + DL |
| 17 | endolymphatic hydrops | KG + DL |

## Disclaimer

These predictions are for research purposes only and do not constitute medical advice.
Clinical validation is required before any clinical application.

---

[← Back to Drug Search](/drugs/)
